...
def explore_chats():
    ...
    chats = chats.join(Sub).group_by(Chat.id).order_by(func.count().desc())
...
def downvote(post_id):
    ...
    if post is None:
        flash('Post not found.')
        return redirect(url_for('main.index'))
    vote = current_user.downvote(post)
    db.session.merge(vote)
    db.session.commit()
